...
export default create<GameStoreProps>((set, get) => ({
    ...
    flip: async (row: number, col: number) => {
        const game = InMemoryGameOfLifeHistory.INSTANCE.flip(get().gameId, { row, col})
        set({board: game?.statuses, population: game?.population()})
    },
    next: async (times: number) => {
        const game = InMemoryGameOfLifeHistory.INSTANCE.nextGame(get().gameId, times)
        set((state) => ({board: game?.statuses, population: game?.population(), generation: state.generation + times}))
    },
    reset: async () => {
        ...
        set({gameId: gameId, board: game?.statuses, generation: 1, population: game?.population()})
    },
    start: async () => {
        const gameId = InMemoryGameOfLifeHistory.INSTANCE.startGame()
        const game = InMemoryGameOfLifeHistory.INSTANCE.retrieveGame(gameId)
        set((state) => ({board: game?.statuses, gameId: gameId, population: game?.population(), generation: state.generation + 1}))
    }
}))